...
const Home = () => {
  const { studentToken, studentNumber, studentData, setStudentData, eventData} = useUser();
  ...
  const registerEvent = studentData?.studentUpcomingEvents || [];
    ...
    <LinearbackGround>
      <SafeAreaView style={styles.safeAreaView}>
        ...
        <View style={{ minHeight: 250 }}>
          {hasEventRegister && registerEvent.length > 0 ? (
            <>
              <Animated.FlatList
                data={registerEvent}
                  ...
                  >
                    <View style={styles.page}>
                      ...
                      <View style={styles.pageInfoRow}>
                        ...
                        <AntDesign name="clockcircleo" size={17} color={COLORS.textColorWhite} />
                        <Text style={styles.pageConatinerText}>{item.time}</Text>
                        <Entypo name="location" size={17} color={COLORS.textColorWhite} />
                        <Text style={styles.pageConatinerText}>{item.location}</Text>
                      </View>
                    </View>
                  </ImageBackground>
                )}
              ...
              <View style={styles.pagination}>
                {registerEvent.map((_: any, i: number) => {
          ...
          renderItem={({ item}: {item: Event}) => (
            <View style={styles.upcomingEventView}>
              <View style={{ flexDirection: "row" }}>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontWeight: "600", fontSize: 19 }}>{item.eventTitle}</Text>
                  <Text>{item.eventOrganizer + " " + item.eventLocation}</Text>
                </View>
                <AntDesign name="checkcircle" size={24} color={COLORS.Primary} style={{ margin: "auto" }} />
              </View>
...
const styles = StyleSheet.create({
  ...
  headContainer: { flexDirection: "row", justifyContent: "space-around", alignItems: "center", margin: 10 },
  profileCircle: { width: 60, height: 60, borderRadius: 30, backgroundColor: "#333", alignItems: "center", justifyContent: "center" },
  profileText: { color: "#fff", fontWeight: "700", fontSize: 20 },
  headerText: { flex: 1, marginLeft: 10 },
  greeting: { fontSize: 12, color: "#666" },
  name: { fontSize: 16, fontWeight: "bold", color: "#222" },
  icon: { marginHorizontal: 5 },
  notificationBadge: { width: 20, height: 20, right: 5, position: "absolute", backgroundColor: "black", borderRadius: 40, zIndex: 10, justifyContent: "center", alignItems: "center" },
  notificationText: { color: "white", fontSize: 12 },
  eventTextTitle: { fontSize: 20, fontWeight: "500", margin: 10 },
  page: { width: width - 20, maxWidth: 500, marginHorizontal: 10, height: 250, borderRadius: 10, overflow: "hidden", justifyContent: "flex-end" },
  pageTitle: { color: COLORS.textColorWhite, fontSize: 24, fontWeight: "600" },
  pageInfoRow: { flexDirection: "row", gap: 10, flexWrap: "wrap", marginBottom: 10 },
  ...
  upcomingEventView: { marginVertical: 7, paddingVertical: 15, paddingHorizontal: 10, borderRadius: 10, backgroundColor: COLORS.Third },
  dividerLine: { height: 3, marginVertical: 10, backgroundColor: "grey", borderRadius: 10 },
});
